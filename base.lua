...
function courseplay:load(savegame)
	...
	self.cp.fillLevelChangeDelay = 10;
...
function courseplay:loadVehicleCPSettings(xmlFile, key, resetVehicles)
	if not resetVehicles and g_server ~= nil then
		...
		-- MODE 3
		curKey = key .. '.courseplay.overloader';
		self.cp.fillLevelChangeDelay  = Utils.getNoNil( getXMLInt(xmlFile, curKey .. '#fillLevelChangeDelay'),	 10);
...
function courseplay:getSaveAttributesAndNodes(nodeIdent)
	...
	local overloader = string.format('<overloarder fillLevelChangeDelay="%d" />', self.cp.fillLevelChangeDelay);
	...
	nodes = nodes .. nodeIdent .. indent .. overloader .. '\n';
